...
"""
win = bid * rank
order hands by strength, rank = 1 for the weakest hand -- n for strongest.
prolly make it hex? for easy comparison if nothing else?
"""

hands = { 'high':{}, 'pair':{}, 'twopair':{}, 'three':{}, 'fully':{}, 'four':{}, 'five':{} }
with open('input') as file:
    for line in file:
        ...
        for i in range(5):
            hand[hand_raw[i]] += 1
        maxlen = 0
        for i in hand: maxlen = max(maxlen, hand[i])
        if maxlen == 5:
            hands['five'][hand_raw] = bid_raw
        elif maxlen == 4:
            hands['four'][hand_raw] = bid_raw
        elif maxlen == 3:
            combo = False
            for i in hand:
                if hand[i] == 2:
...
for t in hands:
    ...
    # helper dict with value -> original? need to look up bids.
    # go through sorted list lowest to max, assigning rank on the way, adding rank * bid...
    lookup = {}
    tosort = []
    for hand in hands[t]:
